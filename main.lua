...
-- Completing theme, layout, and effects
...
local theme = {
	...
	font = 2,                                      -- Cleaner font (Drawing.Fonts.Plex)
...
titleBar.ZIndex = 998
titleBar.Radius = theme.radius -- Rounded edges
...
-- Update createDrawing to support radius
local originalCreateDrawing = createDrawing
createDrawing = function(type, properties, add)
	local drawing = originalCreateDrawing(type, properties, add)
	if type == "Square" and properties and not properties.Radius then
		drawing.Radius = theme.radius
	end
	return drawing
end
...
local function overrideOptionMethod(methodName, createFunc)
...
overrideOptionMethod("AddButton", function(name, func) end)
overrideOptionMethod("AddToggle", function(prop) end)
overrideOptionMethod("AddSlider", function(prop) end)
overrideOptionMethod("AddDropdown", function(prop) end)
